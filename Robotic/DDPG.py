...
class DDPG:
    ...
    def update_models(self, states_and_goals, actions, critic_target):
        """ Update actor and critic networks from sampled experience
        """
        # Train critic
        self.critic.train_on_batch(states_and_goals, actions, critic_target)
        # Q-Value Gradients under Current Policy
        actions = self.actor.model.predict(states_and_goals)
        grads = self.critic.gradients(states_and_goals, actions)
        # Train actor
        self.actor.train(states_and_goals, actions, np.array(grads).reshape((-1, self.act_dim)))
