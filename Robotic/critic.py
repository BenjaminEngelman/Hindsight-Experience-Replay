...
class Critic:
    ...
    def network(self):
        ...
        """
        state_goal = Input((self.env_dim))
        action = Input((self.act_dim,))
        x = Dense(256, activation='relu')(state_goal)
        x = concatenate([Flatten()(x), action])
        x = Dense(128, activation='relu')(x)
        out = Dense(1, activation='linear', kernel_initializer=RandomUniform())(x)
        return Model([state_goal, action], out)

    def gradients(self, states_goals, actions):
        """ Compute Q-value gradients w.r.t. states and policy-actions
        """
        return self.action_grads([states_goals, actions])
    ...
    def train_on_batch(self, states_goals, actions, critic_target):
        """ Train the critic network on batch of sampled experience
        """
        return self.model.train_on_batch([states_goals, actions], critic_target)
